...
ASTnode *prefix(void)
{
    ...
    case T_AMPER:
        ...
        n = prefix();
        if (n->op != A_IDENT)
            custom_error_int("& operator must be followed by an identifier", n->op);
        n->op = A_ADDR;
    ...
    case T_STAR:
        scan(&t_instance);
        n = prefix();
        if (n->op != A_DEREF && n->op != A_IDENT)
            custom_error_int("* operator must be followed by an identifier or *", n->op);
        n = mkAST_left(A_DEREF, value_at(n->type), n, 0);
        break;
    default:
        n = primary();
        break;
